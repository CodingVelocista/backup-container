...
function usage () {
  ...
  Automated backup script for MS SQL Server databases.
...
function readConf(){ 
  (
	...
	if [ -z "${_value}" ] && [ -z "${readCron}" ]; then
	  ...
	  _value="${DATABASE_SERVICE_NAME}/${MSSQL_DATABASE}"
	fi
...
function getUsername(){
  (
	...
	# Backward compatibility ...
	_username="${!_paramName:-${MSSQL_USER}}"
	echo ${_username}
  )
}
...
function getPassword(){
  (
	...
	# Backward compatibility ...
	_password="${!_paramName:-${MSSQL_PASSWORD}}"
	echo ${_password}
  )
}
...
function backupDatabase(){
  (
	...
	_backupFile="${_fileName}${BACKUP_FILE_EXTENSION}"

	echo "Backing up $_database on $_hostname to filename $_backupFile"
...
function restoreDatabase(){
  (
	...
	export MSSQLPASSWORD=${_adminPassword}
	local startTime=${SECONDS}

	# Restore
	if (( ${_rtnCd} == 0 )); then
	  echo "Restoring from backup ..."
	#   gunzip -c "${_fileName}" | psql -v ON_ERROR_STOP=1 -x -h "${_hostname}" -d "${_database}"
		sqlcmd -S $_hostname -U ${_username} -P ${_password} -d master –Q "RESTORE DATABASE $_database FROM DISK='$_fileName'"
...
function generateFilename(){
  (
	...
	_backupType=${FULL_DATABASE_FILE_NAME}
	if [ "$BACKUP_LOG_ONLY" == "true" ]; then
		_backupType=${DATABASE_LOG_FILE_NAME}
	fi;

	_filename="${_backupDir}${_coreFilename}${_backupType}_`date +\%Y-\%m-\%d_%H-%M-%S`"
	echo ${_filename}
  )
}
...
function runBackups(){
  (
	...
	for database in ${databases}; do
	  ...
	  if (( ${rtnCd} == 0 )); then
		backupPath=$(gzipBackup "${filename}")
...
function verifyBackup(){
  (
	...
	if (( ${rtnCd} == 0 )); then
	  _hostname="127.0.0.1"
	  _database=$(getDatabaseName ${_databaseSpec})
	  tables=$(psql -h "${_hostname}" -d "${_database}" -t -c "SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE';")
	  rtnCd=${?}
	fi
...
function getDbSize(){
  (
	...
	if isInstalled "psql"; then
	  size=$(MSSQLPASSWORD=${_password} psql -h "${_hostname}" -U "${_username}" -d "${_database}" -t -c "SELECT pg_size_pretty(pg_database_size(current_database())) as size;")
	  rtnCd=${?}
	else
	  size="not found"
	  rtnCd=1
...
export FULL_DATABASE_FILE_NAME="-FULL-DATABASE"
export DATABASE_LOG_FILE_NAME="-TRANSACTION-LOG"
